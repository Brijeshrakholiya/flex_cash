 $(document).ready(function () {
        ...
        if ($('#refund').length > 0) {
               var $url = BASEURL + $('#refund').attr('data-control') + '/' + $('#refund').attr('data-method');
     ...
     $(document).on("click", ".return_req", function (event) {
         ...
         var $type = $(this).attr('data-type');
            ...
            if($type == 1){
                $.alert.open('confirm', 'Are you sure you want Refund Payment?', function(button) {
                    if (button == 'yes') {
                        $.alert.open({
                        type: 'prompt',
                        ...
                        callback: function(pass_btn, value) {
                            if (pass_btn == 'ok'){
                                $.ajax({
                                    type: 'POST',
                                    url: BASEURL+'refundmst/update_refund_request',
                                    async: false,
                                    data: {id: $id,type: $type,pass: value},
                                    dataType: 'json',
                                    success: function (returnData) {
                                        if (returnData.status == "ok") {
                                            if(returnData.type == 1){
                                                toster_message('Refund is Sent to Customer Successfully.', 'Success', 'success');
                                                oTable.fnDraw();
                                            }else{
                                                toster_message('Refund Request Denied Successfully.', 'Success', 'success');
                                            }
                                            $(".bs-example-modal-md").modal('hide');
                ...
                success: function (returnData) {
                    if (returnData.status == "ok") {
                        if(returnData.type == 1){
                            ...
                            toster_message('Request Denied Successfully.', 'Success', 'success');
                            oTable.fnDraw();
                        }
                        $(".bs-example-modal-md").modal('hide');
                    }else if(returnData.status == "error"){
    ...
    $(document).on("click", ".view_details", function (event) {
        var $id = $(this).attr('data-id');
            ...
            $.ajax({
                type: 'POST',
                url: BASEURL+'refundmst/requestdtl',
                async: false,
                data: {id: $id},
                dataType: 'json',
                beforeSend: function () {
                ...
                success: function (returnData) {
                    if (returnData.status == "ok") {
                        if (typeof returnData.req_info != "undefined") {
                            ...
                            html_data += 
                                '<tr>'
                                    +'<td style="width:30% !important;"><b>Refund Request For</b></td>'
                                    +'<td>'+returnData.req_info.FlexName+'</td>'    
                                +'</tr>'
                                +'<tr>'
                                    +'<td style="width:30% !important;"><b>Refund Request By</b></td>'
                                    +'<td>'+returnData.req_info.username+'</td>'    
                                +'</tr>'
                                +'<tr>'
                                    +'<td style="width:30% !important;"><b>User Join On</b></td>'
                                    +'<td>'+returnData.join_info.JoinDate+'</td>'    
                                +'</tr>'
                                +'<tr>'
                                    +'<td style="width:30% !important;"><b>Join Quentity</b></td>'
                                    +'<td>'+returnData.join_info.Qty+'</td>'    
                                +'</tr>'
                                +'<tr>'
                                    +'<td style="width:30% !important;"><b>Refundable Amount</b></td>'
                                    +'<td>'+returnData.join_info.TxAmt+'</td>'    
                                +'</tr>'
                                +'<tr>'
                                    +'<td style="width:30% !important;"><b>Transaction ID</b></td>'
                                    +'<td>'+returnData.req_info.TransactionID+'</td>'    
                                +'</tr>'
                                +'<tr>'
                            ...
                            if(returnData.req_info.Status == 1){
                                ...
                                btn_data += 
                                    '<button type="button" class="btn btn-success return_req" data-id ='+returnData.req_info.RequestID+' data-type=1 >Approve</button>'
                                    +'<button type="button" class="btn btn-danger return_req" data-id ='+returnData.req_info.RequestID+' data-type=2 >Denied</button>';
                            }
