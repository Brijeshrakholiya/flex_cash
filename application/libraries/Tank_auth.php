...
class Tank_auth
{
	 ...
	 * @return	array
	 */
	function create_user($username, $email, $carrierCode, $phone, $image ,$password, $email_activation)
	{
		...
		/*if ((strlen($username) > 0) AND !$this->ci->users->is_username_available($username)) {
			...
			if (!is_null($res = $this->ci->users->create_mobile_user($data, !$email_activation))) {
	...
	function change_password($old_pass, $new_pass)
	{
        ...
        function mobile_change_password($user_id,$old_pass, $new_pass)
	{
		//$user_id = $this->ci->session->userdata('user_id');
